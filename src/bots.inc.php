...
function bot_init() { 
	...
	try {
		...
		$botman->hears('hello', function (BotMan $bot) { bot_start($bot); } );
		$botman->hears('start', function (BotMan $bot) { bot_start($bot); } );
		$botman->hears('hi', function (BotMan $bot) { bot_start($bot); } );
...
function bot_start($bot) {
	...
	if($user){
		...
		if($uname) $bot->reply('Your username is: '.$uname);
		if($uid) $bot->reply('Your ID is: '.$uid);
	} else $bot->reply('Hello yourself.');
