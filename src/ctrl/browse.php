...
function respondents_browse($page, $sort_by, $sorting)
{
    ...
    if ($bv->db_type == 'postgres') {
        $params = [ $bv->questionnaire_id, $paginator->getPerPage(), $paginator->getStartIndex() ];
    } else {
        $params = [ $bv->questionnaire_id, $paginator->getStartIndex(), $paginator->getPerPage() ];
    }
