...
$app->match('/question', function (Request $request) use ($app) {
    ...
    if (!$bv->questionnaire->id && !$_GET['id']) {
    ...
    if ($bv->question_id) {
        ...
        if (is_numeric($_GET['after'])) {
            $bv->current_step = $_GET['after']+1;
        } elseif (is_numeric($_GET['before'])) {
            $bv->current_step = $_GET['before']-1;
        } else {
            $bv->current_step = is_numeric($_GET['step']) ? $_GET['step'] : $app['session']->get('current_step');
    ...
    if (!count($bv->questions)) {
        $bv->questions = questionnaire_questions($bv->questionnaire->id);
    } // fallback to load all questions
    ...
    foreach ($bv->questions as $bv->question) {
        ...
        if (!$bv->questionnaire) {
            $bv->questionnaire = $bv->question->questionnaire;
        } // get from table
        ...
        if ($bv->question->continue_label) {
            $bv->questionnaire->continue_label = $bv->question->continue_label;
